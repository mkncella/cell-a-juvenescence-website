...
@section('content')
    ...
    <section x-show="true" @click="console.log('dorrrr')" class="">
        <div class="flex justify-center -mt-12 mb-8">
            <div class="flex bg-gray-200 rounded-xl p-2 shadow-lg border">
                <button type="button"
                    class="px-8 py-4 rounded-lg font-semibold text-lg transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-blue-300 focus:ring-offset-2 min-w-[140px] relative" 
                    :class="activeTab === 'join' ? 'bg-white text-blue-700 shadow-md transform scale-105' : 'text-gray-700 hover:text-gray-900 hover:bg-gray-100'"
                    @click="console.log('Join Us button clicked'); setActiveTab('join'); "
                >
                    Join Us
                    <div x-show="activeTab === 'join'" class="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-8 h-1 bg-blue-600 rounded-full"></div>
                </button>
                <button 
                    class="px-8 py-4 rounded-lg font-semibold text-lg transition-all duration-300 focus:outline-none focus:ring-4 focus:ring-blue-300 focus:ring-offset-2 min-w-[140px] relative" 
                    :class="activeTab === 'official' ? 'bg-white text-blue-700 shadow-md transform scale-105' : 'text-gray-700 hover:text-gray-900 hover:bg-gray-100'"
                    @click="setActiveTab('official'); console.log('Official Reseller button clicked')"
                >
                    Official Reseller
                    <div x-show="activeTab === 'official'" class="absolute bottom-0 left-1/2 transform -translate-x-1/2 w-8 h-1 bg-blue-600 rounded-full"></div>
                </button>
            </div>
        </div>


        <div class="" @click="console.log('halo')" x-data="indonesiaMapData" x-init="
            stores = @js($stores);
            rewards = @js($rewards);
            activeTab = getQueryParam('tab') || localStorage.getItem('activeTab') || 'join';
            localStorage.setItem('activeTab', activeTab);
            console.log('Alpine.js initialized with activeTab:', activeTab);
            console.log('setActiveTab function available:', typeof this.setActiveTab);
        ">          
            <section x-show="activeTab === 'join'">
            ...
            <section x-ref="officialReseller" class="pt-8" x-show="activeTab === 'official'">
    ...
    @push("script")
        ...
        <script>
            ...
            // Define the indonesiaMapData function
            window.indonesiaMapData = function() {
                return {
                    stores: [],
                    rewards: [],
                    markers: {},
                    ...
                    init() {
                        ...
                        setTimeout(async () => {
                            ...
                            this.map = L.map('map').setView([-2.5, 118.0], 4);
                            ...
                            stores.forEach((store, i) => {
                                ...
                                marker.on('popupopen', async e => {
                                    ...
                                    // focus to marker
                                    this.map.setView([lat, lng], zoom || 13)
                            ...
                            // add GeoJSON layer for province highlighting
                            try {
                                const response = await fetch('{{ asset("maps/indonesia-province-simple.json") }}')
                                ...
                                this.geoJsonLayer = L.geoJSON(geoJsonData, {
                                    style: (feature) => {
                                        ...
                                        const isSelected = this.regions[provinceName]?.isSelected
                    ...
                    focusMarker(id) {
                        ...
                        // focus to marker
                        const { lat, lng, zoom } = this.stores.find(store => store.id === id)
                        this.map.setView([lat, lng], zoom || 13)

                        // highlight province
                        const { province } = this.stores.find(store => store.id === id)
                        this.highlightProvince(province)
                    },

                    highlightProvince(name) {
                        if (!name || !this.geoJsonLayer) return
                        ...
                        if (this.regions[name]) {
                            this.regions[name].isSelected = true
                        }
