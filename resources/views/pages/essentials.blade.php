...
@section('content')
    ...
    <section class="products-section flex gap-4 p-8" x-data="() => ({
        test: true,
        products: @js($products),
        skinConcerns: @js($skin_concerns),
        filter_concerns: @js($arg_concerns).filter(val => val),
        imageProductPath: '{{ asset('images/products/') . '/' }}',
        now: new Date(),
        ...
        get filteredProducts() {
            const { products, filter_concerns } = this

            let _products = products


            if (filter_concerns.length) {
                _products = _products.filter(({ skin_concerns }) => filter_concerns.every((concern) => skin_concerns.some(({ name }) => name == concern)))
                {{-- skin_concerns.some(({ name: concern_name }) => filter_concerns.includes(concern_name))) --}}
            }
        ...
        lessThan30day(date) {
            ...
            const diffInMs = this.now - _date
            const diffInDays = diffInMs / (1000 * 60 * 60 * 24)
            return diffInDays > 0 && diffInDays < 31
        },
        rupiah(num, isWithRp = false) {
            return (isWithRp ? 'Rp' : '') + this.numberFormat.format(num - 0)
        },
        truncate(text, limit = 100, endPrefix = '...') {
            return text?.slice(0, (limit && !isNaN(limit)) ? limit : 100).concat(endPrefix || '...')
        },
    })">
            ...
            <div
                ...
                <ul class="list-none flex flex-col gap-2 justify-start !pl-0 ml-0">
                    <template x-for="(concern) in skinConcerns" @:key="concern.id">
                        <li class="inline-flex justify-between items-center gap-4 md:!gap-8 group">
                            <button @@click="toggleFilterConcern(concern.name)" class="group p-0.5 text-xs font-normal text-gray-900 !rounded-lg !overflow-hidden bg-gradient-to-br from-green-400 to-blue-600 flex items-center justify-center w-full">
                                <span x-text="concern.name" :class="filter_concerns.includes(concern.name) ? '!bg-transparent text-white' : 'group-hover:!bg-transparent group-hover:text-white'" class="w-full bg-white rounded-md p-0.5 transition-all ease-in duration-75"></span>
                            </button>
                        </li>
                    </template>
                </ul>
                ...
                <template x-for="(product, index) in filteredProducts" x-bind:key="product.id + '' + index">
                    ...
                    <div class="group flex-1 basis-52 max-w-[16rem] rounded-lg p-4 border  relative box-border h-64" x-data="() => ({
                        ...
                        <div class="head flex justify-center">
                            ...
                            <div class="absolute top-0 left-0 right-0 flex justify-between gap-4">
                                <div class="text-white absolute text-xs">
                                    <div class="bg-red-600 text-center py-0.5 px-1" x-bind:class="{ 'hidden': !product.discount }" x-text="product.discount + '%'"></div>
                                    <div class="bg-black text-center py-0.5 px-1" x-bind:class="{ 'hidden': !isNewProduct }">New!</div>
                                </div>
                        ...
                        <div class="body pt-6 transition-all ease-in-out">
                            <p class="text-base font-bold mb-1 duration-300 group-hover:-translate-y-3" x-text="product.name"></p>
                            <p class="text-xs mb-1 duration-300 group-hover:-translate-y-3 ease-in" x-text="truncate(product.description, 50)"></p>
                            <p class="relative flex justify-start items-center font-bold duration-500
                                    group-hover:opacity-0 group-hover:translate-y-4">
                                <span class="text-sm" x-text="rupiah(product.price - (!product.discount ? 0 : (product.price * product.discount / 100)), true)"></span>
                                <span x-show="product.discount" class="text-[10px] text-gray-500 line-through ml-2" x-text="product.price"></span>
                            </p>
                            <p class="flex flex-wrap items-center gap-1 w-full translate-y-[-2.75rem] opacity-0 ease-in group-hover:!opacity-100 scale-50 group-hover:scale-100 origin-left duration-300 group-hover:delay-[200ms]">
                                <template x-for="(concern, index) in product.skin_concerns" @:key="index">
